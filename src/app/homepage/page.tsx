	...
	return (
			<motion.div initial="hidden" animate="visible" className="flex flex-col overflow-x-hidden overflow-y-scroll">
				<main className="w-full h-screen flex flex-col overflow-hidden transition-all ease-in duration-75" style={{background : theme?.theme.colors.background}}>
					...
					{/* DEBUG						 */}
					<p className="text-xl text-center" style={{color: theme?.theme.colors.text}}>THEME: {theme?.theme.name}</p>
					<button onClick={() => switchT(darkTheme)} style={{color: theme?.theme.colors.text}}>Change theme dark</button>
					<button onClick={() => switchT(defaultTheme)} style={{color: theme?.theme.colors.text}}>Change theme default</button>
					<button onClick={() => switchT(lightTheme)} style={{color: theme?.theme.colors.text}}>Change theme light</button>
					<button onClick={() => switchT(duskBlueTheme)} style={{color: theme?.theme.colors.text}}>Change theme dusk</button>
					{/* 							 */}
						...
						<div style={{background: theme?.theme.colors.primary}} className=" pb-10 pt-2 w-full mt-10 flex-col flex items-center">
							<div className="w-11/12 max-w-[1200px] h-full translate-y-4">
								<motion.div variants={slideInFromLeft(0.5)} className="flex flex-row">
									<h1 className="text-4xl font-semibold italic" style={{color: theme?.theme.colors.text_secondary}}>Why </h1>
									<LOGO width="180px" height="60px" white={true} />
									<h1 className="text-4xl font-semibold italic" style={{color: theme?.theme.colors.text_secondary}}>?</h1>
								</motion.div>
								<motion.div variants={slideInFromRight(0.5)} className="flex flex-row flex-wrap -mt-4 justify-center">
									<div className="flex flex-col mt-5 mr-16 opacity-75 select-none">
										<div className="flex flex-row justify-start items-center">
											...
											<span className="font-bold italic text-5xl ml-2" style={{color: theme?.theme.colors.text_secondary}}>FAST</span>
										</div>
									...
									<div className="flex flex-col mt-5 mr-16 opacity-75 select-none">
										<div className="flex flex-row justify-start items-center">
											...
											<span className="font-bold italic text-5xl ml-2" style={{color: theme?.theme.colors.text_secondary}}>SECURE</span>
										</div>
										<div className="flex flex-col text-sm mt-1 max-w-[300px]" style={{color: theme?.theme.colors.text_secondary}}>
						...
						<div style={{background: theme?.theme.colors.primary}} className=" pb-5 pt-2 w-full mt-2 flex-col flex items-center">
							<div className="w-11/12 max-w-[1200px] h-full flex flex-col justify-center items-start">
								<div className="flex-row flex">
									<h1 className="font-bold italic text-6xl h-fit" style={{color: theme?.theme.colors.text_secondary}}>GET YOUR SPARK ACCOUNT NOW</h1>
								</div>
								<div className="flex-row flex justify-center items-center">
									<h1 className="font-bold italic text-2xl h-fit" style={{color: theme?.theme.colors.text_secondary}}>Contact us</h1>
...
const Card: React.FC<CardProps> = ({ title, content }) => {
	...
	return (
		<div style={{background: theme?.theme.colors.primary, borderColor: theme?.theme.colors.accent}} className="border border-spacing-1.5 hover:opacity-100 opacity-75 text-white w-64 min-h-20 pb-5 rounded-md flex flex-col items-center cursor-pointer transition-all ease-in duration-75 hover:translate-x-1 hover:-translate-y-2 hover:z-10 hover:shadow-lg">
			<h1 style={{color: theme?.theme.colors.text}} className="flex flex-row p-2 text-2xl font-bold">{title}</h1>
			<span style={{color: theme?.theme.colors.text}} className="text-center p-1 pt-0 font-semibold">{content}</span>
		</div>
	);
};
