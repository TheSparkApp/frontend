...
export default function PostPage({ params }: Params) {
	...
	const formattedTimestamp = (
		<DateFormatter timestamp={parseInt("1715519669")} />
	); // OUT OF DATABASE
	...
	return (
		<div className="flex flex-col overflow-x-hidden overflow-y-hidden">
			<main
				...
				<div
					style={{ color: theme?.theme.colors.text }}
					className="flex flex-col h-full w-full max-w-[1200px] pl-5 pr-5 break-words">
					<p>Content</p>
					...
					<div className="flex flex-col justify-center">
						<h1 className="text-xl break-words">
							...
							<a
								...
								href={`/home/users/${Author.id}`}>
								{Author.name}
							</a>
						</h1>
						<div className="flex flex-row">
							<p className="opacity-75">{formattedTimestamp}</p>
							<p className="opacity-75 ml-5 flex flex-row items-center gap-1">
								<FaEye />
								{formattedViews}
							</p>
							<p className="opacity-75 ml-5 flex flex-row">
								ID {blogId}
							</p>
						</div>
					</div>
