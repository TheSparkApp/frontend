...
export const NavBar = () => {
	...
	return (
		<div
			...
			className="w-full backdrop-blur-lg h-12 max-h-12 min-h-12 p-1 flex overflow-y-hidden flex-row fixed z-50 justify-start items-center border-b border-gray-900 border-spacing-1.5">
			<div
				onClick={() => router.push("/")}
			...
			<div className="w-full flex flex-row items-center justify-center select-none">
				...
				<div className="w-fit mr-10  relative h-full flex-row flex justify-end items-center">
					<div
						style={{ borderColor: theme?.theme.colors.accent }}
						className="absolute bg-transparent border border-spacing-2 no-underline p-3 pt-1 ml-5 ease-in duration-150 transition-all hover:bg-yellow-500 pb-1 cursor-pointer rounded-md">
						<a
							onClick={(e) => {
								e.preventDefault()
								router.push(
							...
							style={{ color: theme?.theme.colors.text, textDecoration: 'none' }}
